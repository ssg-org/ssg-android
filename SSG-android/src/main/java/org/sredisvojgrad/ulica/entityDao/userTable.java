...
public class userTable extends TableHelper<user> {
    ...
    public void bindRowValues(DatabaseUtils.InsertHelper insHelper, String[] rowValues) {
        if (rowValues[0] == null) insHelper.bindNull(0);
        else insHelper.bind(0, LongConverter.GET.fromString(rowValues[0]));
        if (rowValues[1] == null) insHelper.bindNull(1);
        else insHelper.bind(1, StringConverter.GET.fromString(rowValues[1]));
        if (rowValues[2] == null) insHelper.bindNull(2);
        else insHelper.bind(2, StringConverter.GET.fromString(rowValues[2]));
        if (rowValues[3] == null) insHelper.bindNull(3);
        else insHelper.bind(3, StringConverter.GET.fromString(rowValues[3]));
        if (rowValues[4] == null) insHelper.bindNull(4);
        else insHelper.bind(4, StringConverter.GET.fromString(rowValues[4]));
        if (rowValues[5] == null) insHelper.bindNull(5);
        else insHelper.bind(5, StringConverter.GET.fromString(rowValues[5]));
    }
